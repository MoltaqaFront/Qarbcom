    ...
    <main>
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.providers-join-request") }}</h5>
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.company_name`]="{ item }">
          <h6 class="text-danger" v-if="!item.company_name"> - </h6>
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/providers-join-request/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    async HandlingItemActivationStatus(selectedItem) {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `joinProviders/reject/${targetItem.id}`,
          data: REQUEST_DATA,
        });
    ...
    // ===== Start:: Show
    showItem(item) {
      this.$router.push({ path: `/providers-join-request/show/${item.id}` });
    },
    // ===== End:: Show
