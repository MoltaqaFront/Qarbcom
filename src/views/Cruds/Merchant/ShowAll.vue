      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            ...
            <a-tooltip placement="bottom" v-if="$can('providers delete', 'providers')">
              <template slot="title">
            ...
            <template v-if="$can('providers active', 'providers')">
  ...
  methods: {
    ...
    async HandlingItemActivationStatus(selectedItem) {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `allProviders/active/${targetItem.id}`,
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `allProviders/${this.itemToDelete.id}`,
        });
