...
export default {
  ...
  methods: {
    ...
    async getDataToEdit() {
      try {
        let res = await this.$axios({
          method: "GET",
          url: `settings/contact-with-management`,
        });
        // Start:: Set Data

        // Transform the API response

        this.phones = res.data.data.setting.value.phones.map(phone => ({ phone: phone }));

        this.data.WhatsApp_contact = res.data.data.setting.value.watsApp;
    ...
    async submitForm() {
      ...
      REQUEST_DATA.append("key", "dashboard-admin-contact-with-management");
