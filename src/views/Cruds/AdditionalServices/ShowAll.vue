    ...
    <main>
        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <!-- Start:: Name Input -->
              <base-input col="6" type="text" :placeholder="$t('TABLES.Services.name')"
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.additional_services") }}</h5>
        ...
        <div class="title_route_wrapper" v-if="$can('additionalServices create', 'additionalServices')">
          <router-link to="/additionalServices/create">
            {{ $t("SIDENAV.Services.add") }}
          </router-link>
        </div>
      </div>
      <!--  =========== End:: Table Title =========== -->
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.is_active`]="{ item }">
          <div class="activation" dir="ltr" style="z-index: 1"
            v-if="$can('additionalServices activate', 'additionalServices')">
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            ...
            <a-tooltip placement="bottom" v-if="$can('additionalServices show', 'additionalServices')">
              <template slot="title">
            ...
            <a-tooltip placement="bottom" v-if="$can('additionalServices edit', 'additionalServices')">
              <template slot="title">
            ...
            <a-tooltip placement="bottom" v-if="$can('additionalServices delete', 'additionalServices')">
              <template slot="title">
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("TABLES.Services.name"),
          value: "name",
          sortable: false,
          align: "center",
        },
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/additionalServices/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "GET",
          url: `additionalServices/active/${item.id}`,
        });
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/additionalServices/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/additionalServices/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `additionalServices/${this.itemToDelete.id}`,
        });
