    ...
    <main>
        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <!-- Start:: Name Input -->
              <base-input col="3" type="text" :placeholder="$t('PLACEHOLDERS.employee_name')"
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.providers-employee") }}</h5>
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.code`]="{ item }">
          <h6 class="text-danger" v-if="!item.code"> - </h6>
          <h6 v-else> {{ item.code }} </h6>
        </template>
        <!-- End:: code -->
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("PLACEHOLDERS.employee_name"),
          value: "name",
          align: "center",
          sortable: false,
        },
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/providers-employee/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
      ...
      try {
        ...
        let res = await this.$axios({
          ...
          params: {
            ...
            employeeProviderCode: this.filterOptions.provider_code,
    ...
    // ===== Start:: Show
    showItem(item) {
      this.$router.push({ path: `/providers-employee/show/${item.id}` });
    },
    // ===== End:: Show
