    ...
    <main>
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.vechile_type") }}</h5>
        ...
        <div class="title_route_wrapper" v-if="$can('vehicleTypes create', 'vehicleTypes')">
          <router-link to="/vehicleTypes/create">
            {{ $t("PLACEHOLDERS.add_vechile") }}
          </router-link>
        </div>
      </div>
      <!--  =========== End:: Table Title =========== -->
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.is_active`]="{ item }">
          <div class="activation" dir="ltr" style="z-index: 1" v-if="$can('vehicleTypes activate', 'vehicleTypes')">
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            ...
            <a-tooltip placement="bottom" v-if="$can('vehicleTypes show', 'vehicleTypes')">
              <template slot="title">
            ...
            <a-tooltip placement="bottom" v-if="$can('vehicleTypes edit', 'vehicleTypes')">
              <template slot="title">
            ...
            <a-tooltip placement="bottom" v-if="$can('vehicleTypes delete', 'vehicleTypes')">
              <template slot="title">
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/vehicleTypes/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "GET",
          url: `vehicleTypes/active/${item.id}`,
        });
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/vehicleTypes/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/vehicleTypes/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `vehicleTypes/${this.itemToDelete.id}`,
        });
